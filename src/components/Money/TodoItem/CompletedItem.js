...
import {M_check_Completed_Item, M_delete_Completed_Item } from "./SliceCompletedItem";
...
const CompletedItem = ({completedModalContent})=>{
    ...
    return (
      ...
      <img onClick={()=>dispatch(M_check_Completed_Item())} className="check" src = {item.isDone ? check : noncheck} alt="icon"></img>
      </TitleContainer>
      <ContentContainer>
      <div className="content">{item.content}</div>
      <img onClick={()=>dispatch(M_delete_Completed_Item(item.id))} className='delete' src={del} alt="icon"></img>
      </ContentContainer>
