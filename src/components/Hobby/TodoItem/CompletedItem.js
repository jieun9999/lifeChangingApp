...
import {H_check_Completed_Item, H_delete_Completed_Item } from "./SliceCompletetdItem";
...
const CompletedItem = ({completedModalContent})=>{
    ...
    return (
      ...
      <img onClick={()=>dispatch(H_check_Completed_Item())} className="check" src = {item.isDone ? check : noncheck} alt="icon"></img>
      </TitleContainer>
      <ContentContainer>
      <div className="content">{item.content}</div>
      <img onClick={()=>dispatch(H_delete_Completed_Item(item.id))} className='delete' src={del} alt="icon"></img>
      </ContentContainer>
